...
export default function LocationPicker({ onLocationSet }) {
  ...
  return (
    <div className="w-full max-w-md mx-auto animate-fade-in-up relative z-10">
      <div className="bg-[#faf6f1] dark:bg-neutral-900 rounded-2xl p-4 sm:p-6 border border-amber-200 dark:border-neutral-800 shadow-sm">
        <button
        ...
        <div className="relative mt-4">
          <div className="flex items-center gap-3 text-stone-500 dark:text-neutral-600 text-xs font-medium uppercase tracking-wider">
            <div className="flex-1 h-px bg-amber-200 dark:bg-neutral-800" />
          ...
          <input
            ...
            placeholder="Where are you hungry?"
            className="mt-3 w-full bg-white dark:bg-neutral-800 border border-amber-200 dark:border-neutral-700 rounded-xl py-3.5 px-4 text-stone-800 dark:text-neutral-200 placeholder-stone-400 dark:placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
          />

          {suggestions.length > 0 && (
            <ul className="absolute z-50 w-full mt-1 bg-white dark:bg-neutral-800 border border-amber-200 dark:border-neutral-700 rounded-xl shadow-lg max-h-60 overflow-y-auto">
              {suggestions.map((s, i) => (
                <li key={i}>
                  <button
                    onClick={() => handleSelectSuggestion(s)}
                    className="w-full text-left px-4 py-3 hover:bg-orange-50 dark:hover:bg-neutral-700 text-sm text-stone-700 dark:text-neutral-300 border-b border-amber-100 dark:border-neutral-700/50 last:border-b-0 cursor-pointer transition-colors"
                  >
                    {s.displayName}
                  </button>
                </li>
